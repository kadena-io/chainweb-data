...
do
   sqlite3 -header -csv $HOME/.local/share/chainweb-node/mainnet01/0/sqlite/pact-v1-chain-$i.sqlite "select rowkey as acct_id, txid, cast(ifnull(json_extract(rowdata, '$.balance.decimal'), json_extract(rowdata, '$.balance')) as REAL) as 'balance'
